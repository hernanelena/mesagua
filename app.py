...
<style>
    ...
    .sidebar-ficha {
        background-color: #f8f9fa; padding: 15px; border-radius: 10px; border: 1px solid #e0e0e0;
    }
    .ficha-header {
        background-color: #1E3A8A; color: white; padding: 10px; border-radius: 5px;
        text-align: center; margin-bottom: 15px; font-weight: bold;
    }
    @media (max-width: 600px) { .titulo-responsive { font-size: 1.4rem; } }
</style>
...
def mapear_nombres_claros(valor, tipo):
    ...
    mapeos_maestros = {
        "asistencia": {
            "ong": "ONG",
            "instituci_n_nacionales": "Nación",
            "instituci_n_provinciales": "Provincia",
            "propio": "Propio",
            "otros": "Otros",
        ...
        "usuario": {
            ...
            "com_ind": "Comununidad indígena",
            "escuelas": "Escuela",
            "prod_af": "Familia rural criolla",
            "furbanas": "Familia urbana"
    ...
    # Si el valor técnico existe en nuestro diccionario, devuelve el nombre claro
...
def cargar_datos():
    try:
        ...
        if col_fecha:
            df['fecha_limpia'] = pd.to_datetime(df[col_fecha], errors='coerce')
        else:
            df['fecha_limpia'] = pd.to_datetime(datetime.now())
...
# --- CONFIGURACIÓN DE COLORES Y TECNOLOGÍAS ---
mapa_config = {
...
# --- FILTROS (MODIFICADO PARA OCULTARSE EN MÓVILES) ---
if not df_raw.empty:
...
if not df_filtrado.empty:
    ...
    for _, reg in df_filtrado.iterrows():
        ...
        pop_html = f"<div style='font-family:Arial; min-width:180px;'><b style='color:{conf['hex']}'>{titulo_display.upper()}</b><br><b>Fecha:</b> {f_str}<br><b>En Uso:</b> {reg.get('En_uso','-')}<br><b>Nombre:</b> {reg.get('Nombre','-')}</div>"
    ...
    with st.sidebar:
        ...
        if punto_click:
            ...
            for etiqueta, (kws, tipo_mapa) in datos_ficha.items():
                ...
                # Si definimos un tipo de mapa, limpiamos el nombre
                if tipo_mapa:
